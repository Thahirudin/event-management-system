...
class KeuanganController extends Controller
{
    ...
    function update(Request $request, $id)
    {
        try {
            ...
            return redirect()->back()->with('error', 'Gagal mengedit keuangan. ' . $e->getMessage());
        }
    }
    function destroy($id)
    {
    ...
    {
        $keuangans = Keuangan::whereHas('event', function ($query) {
            $query->where('id_organizer', Auth::user()->id);
        })->get();
        return view('organizer.keuangan', compact('keuangans'));
    }
    ...
    {
        $event = Event::find($id);
        return view('organizer.tambah-keuangan', compact('event'));
    }
    function organizerStore(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect(route('organizer-list-keuangan'))->withInput()->with('sukses', 'Data berhasil disimpan');
        } catch (\Exception $e) {
            DB::rollBack(); // Rollback transaksi database dalam kasus exception
            // Tangani exception, log, atau kembalikan respons error
        }
    ...
    {
        $keuangan = Keuangan::find($id);
        return view('organizer.edit-keuangan', compact('keuangan'));
    }
    function organizerUpdate(Request $request, $id)
    {
        try {
            ...
            return redirect('/organizer/keuangan/list-keuangan')->with('sukses', 'Keuangan Berhasil DiEdit');
        } catch (\Exception $e) {
            DB::rollBack(); // Rollback the transaction in case of an exception
            return redirect()->back()->with('error', 'Gagal mengedit keuangan. ' . $e->getMessage());
        }
    ...
    function organizerDestroy($id)
    {
        ...
        // Hapus data
        $keuangan->delete();
        return redirect('/organizer/keuangan/list-keuangan')->with('sukses', 'Keuangan Berhasil Di Hapus');
    }
    function organizerListPemasukan()
    {
        $keuangans = Keuangan::where('jenis', 'pemasukan')->whereHas('event', function ($query) {
            $query->where('id_organizer', Auth::user()->id);
        })->get();
        return view('organizer.list-pemasukan', compact('keuangans'));
    }
    function organizerListPengeluaran()
    {
        $keuangans = Keuangan::where('jenis', 'pengeluaran')->whereHas('event', function ($query) {
            $query->where('id_organizer', Auth::user()->id);
        })->get();
        return view('organizer.list-pengeluaran', compact('keuangans'));
    }
}